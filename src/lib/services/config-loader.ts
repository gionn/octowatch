  ...
  try {
    ...
    if (typeof window !== "undefined") {
      ...
      if (response.ok) {
        ...
        throw new Error(
          `Failed to fetch config.yaml: ${response.status} ${response.statusText}`,
        );
      }
    }
  ...
  try {
    ...
    if (config.repository_groups) {
      if (!Array.isArray(config.repository_groups)) {
        throw new Error(
          "Invalid configuration: repository_groups must be an array",
        );
      }
      ...
      throw new Error(
        "Invalid configuration: either repository_groups or repositories must be provided",
      );
    }
...
export function getRepositoryGroupBySlug(
  config: Config,
  slug: string,
